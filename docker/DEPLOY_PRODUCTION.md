...
> **Note**: The legacy `phobos-net.py` entrypoint has been removed. Prefer `docker/DEPLOY_TO_SERVER.sh` to deploy the modular `app/` and `frontend/` structure.
...
1. **Copy files to server:**
   ...
   ssh -i ~/.ssh/id_ed25519_192.168.0.73 root@192.168.0.73 "mkdir -p /root/phobos-net/docker/{templates,static,scripts,sample_data}"
   
   # Copy docker files
   scp -i ~/.ssh/id_ed25519_192.168.0.73 docker/docker-compose.yml docker/Dockerfile docker/docker-entrypoint.sh root@192.168.0.73:/root/phobos-net/docker/
   
   # Copy application files
   scp -i ~/.ssh/id_ed25519_192.168.0.73 phobos-net.py threat-feeds.txt requirements.txt root@192.168.0.73:/root/phobos-net/
   scp -i ~/.ssh/id_ed25519_192.168.0.73 -r templates/* root@192.168.0.73:/root/phobos-net/templates/
   scp -i ~/.ssh/id_ed25519_192.168.0.73 -r static/* root@192.168.0.73:/root/phobos-net/static/
   scp -i ~/.ssh/id_ed25519_192.168.0.73 scripts/gunicorn_config.py root@192.168.0.73:/root/phobos-net/scripts/
   scp -i ~/.ssh/id_ed25519_192.168.0.73 -r sample_data root@192.168.0.73:/root/phobos-net/ 2>/dev/null || true
   ```
...
ssh -i ~/.ssh/id_ed25519_192.168.0.73 root@192.168.0.73 "
  ...
  docker exec phobos-net chown root:root /app/phobos-net.py && \
...
# Restart container
docker compose -f /root/phobos-net/docker/docker-compose.yml restart

# Check status
docker compose -f /root/phobos-net/docker/docker-compose.yml ps
