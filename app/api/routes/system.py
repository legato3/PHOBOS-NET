...
from app.services.shared.decorators import (
    throttle,
    login_required,
    admin_required,
    cached_endpoint,
...
def favicon():
    ...
    return send_from_directory(
        "../frontend/src", "favicon.ico", mimetype="image/vnd.microsoft.icon"
    )
    ...
    return jsonify(
        {
            ...
            "cwd": os.getcwd(),
            "static_folder_exists": os.path.exists(current_app.static_folder)
...
def metrics():
    ...
    add_metric(
        "netflow_nfdump_calls_total",
        _metric_nfdump_calls,
        "Total number of nfdump calls",
        "counter",
    )
    add_metric(
        "netflow_stats_cache_hits_total",
        _metric_stats_cache_hits,
        "Cache hits for stats endpoints",
        "counter",
    )
    add_metric(
        "netflow_bw_cache_hits_total",
        _metric_bw_cache_hits,
        "Cache hits for bandwidth endpoint",
        "counter",
    )
    add_metric(
        "netflow_flow_cache_hits_total",
        _metric_flow_cache_hits,
        "Cache hits for flows endpoint",
        "counter",
    )
    add_metric(
        "netflow_http_429_total",
        _metric_http_429,
...
def health_check():
    ...
    all_ok = (
        checks["database"]
        and checks["syslog_active"]
        and checks["nfdump_available"]
        and disk_status != "critical"
    )
...
def api_server_health():
    ...
    with _cache_lock:
        if (
            _server_health_cache["data"]
            and (now - _server_health_cache["ts"]) < SERVER_HEALTH_CACHE_TTL
        ):
        ...
        if "cores" not in data["cpu"]:
            try:
                with open("/proc/cpuinfo", "r") as cf:
                    cores = len(
                        [l for l in cf.readlines() if l.startswith("processor")]
                    )
        ...
        try:
            ...
            if "MemTotal" in meminfo and "MemAvailable" in meminfo:
                ...
                if "MemFree" in meminfo:
                    data["memory"]["free_gb"] = round(
                        meminfo["MemFree"] / (1024 * 1024), 2
                    )
                if "Buffers" in meminfo:
                    data["memory"]["buffers_gb"] = round(
                        meminfo["Buffers"] / (1024 * 1024), 2
                    )
                if "Cached" in meminfo:
                    data["memory"]["cached_gb"] = round(
                        meminfo["Cached"] / (1024 * 1024), 2
                    )
            ...
            if "SwapTotal" in meminfo and "SwapFree" in meminfo:
                ...
                if swap_total_kb > 0:
                    data["memory"]["swap_total_gb"] = round(
                        swap_total_kb / (1024 * 1024), 2
                    )
                    data["memory"]["swap_used_gb"] = round(
                        swap_used_kb / (1024 * 1024), 2
                    )
                    data["memory"]["swap_free_gb"] = round(
                        swap_free_kb / (1024 * 1024), 2
                    )
                    data["memory"]["swap_percent"] = round(
                        (swap_used_kb / swap_total_kb) * 100, 1
                    )
        except Exception as e:
            pass
    except Exception as e:
    ...
    try:
        ...
        data["syslog"] = {
            ...
            "last_event": datetime.fromtimestamp(
                syslog_status.get("last_packet_time")
            ).strftime("%H:%M:%S")
            if syslog_status.get("last_packet_time")
            else None,
            "active": active,
        }
    except Exception:
    ...
    try:
        ...
        data["firewall_syslog"] = {
            ...
            "last_event": datetime.fromtimestamp(fw_last_log).strftime("%H:%M:%S")
            if fw_last_log
            else None,
            "active": fw_active,
        }
    ...
    try:
        ...
        try:
            ...
            if hasattr(
                state, "_stats_flow_stats_cache"
            ) and state._stats_flow_stats_cache.get("data"):
                total_flows = state._stats_flow_stats_cache["data"].get(
                    "total_flows", 0
                )
            ...
            if total_flows == 0:
                ...
                with state._common_data_lock:
                    matches = [
                        v
                        for k, v in state._common_data_cache.items()
        ...
        try:
            result = subprocess.run(
                ["nfdump", "-V"], capture_output=True, text=True, timeout=2
            )
        ...
        try:
            if os.path.exists(nfdump_dir):
                files = [
                    f
                    for f in os.listdir(nfdump_dir)
                    if f.startswith("nfcapd.") and not f.endswith(".current")
                ]
                if files:
                ...
                if files:
                    netflow_data["files_count"] = max(
                        netflow_data["files_count"], len(files)
                    )
        except Exception:
            pass
        ...
        with _firewall_db_lock:
            ...
            try:
                ...
                try:
                    ...
                    cutoff_1h = now - 3600
                    cursor_24h = conn.execute(
                        "SELECT COUNT(*) FROM fw_logs WHERE timestamp > ?",
                        (cutoff_24h,),
                    )
                    count_24h = cursor_24h.fetchone()[0]
                    cursor_1h = conn.execute(
                        "SELECT COUNT(*) FROM fw_logs WHERE timestamp > ?", (cutoff_1h,)
                    )
    ...
    try:
        ...
        try:
            ...
            thread_count = len(
                [d for d in glob.glob(f"/proc/{pid}/task/*") if os.path.isdir(d)]
            )
        ...
        try:
            ...
            data["system"]["boot_time_formatted"] = datetime.fromtimestamp(
                boot_time
...
def api_resource_history():
    ...
    if history:
        ...
        cpu_values = [
            s["cpu_percent"] for s in history if s.get("cpu_percent") is not None
        ]
        mem_values = [
            s["mem_percent"] for s in history if s.get("mem_percent") is not None
        ]
...
def api_dependency_health():
    ...
    if (
        dep_health.get("nfcapd", {}).get("latest_file_age_sec", 0)
        and dep_health["nfcapd"]["latest_file_age_sec"] > 300
    ):
...
def api_container_metrics():
    ...
    if metrics.get("memory_usage_bytes"):
        metrics["memory_usage_mb"] = round(
            metrics["memory_usage_bytes"] / (1024 * 1024), 1
        )
    if metrics.get("memory_limit_bytes"):
        metrics["memory_limit_mb"] = round(
            metrics["memory_limit_bytes"] / (1024 * 1024), 1
        )
    if metrics.get("memory_cache_bytes"):
        metrics["memory_cache_mb"] = round(
            metrics["memory_cache_bytes"] / (1024 * 1024), 1
        )
    ...
    if metrics.get("blkio_write_bytes"):
        metrics["blkio_write_mb"] = round(
            metrics["blkio_write_bytes"] / (1024 * 1024), 2
        )
...
def api_monitoring_summary():
    ...
    if container.get("memory_usage_bytes"):
        container["memory_usage_mb"] = round(
            container["memory_usage_bytes"] / (1024 * 1024), 1
        )
    if container.get("memory_limit_bytes"):
        container["memory_limit_mb"] = round(
            container["memory_limit_bytes"] / (1024 * 1024), 1
        )
    ...
    return jsonify(
        {
            ...
            "dependencies": {
                "nfcapd": {
                    "running": nfcapd_ok,
                    "file_age_sec": dep_health.get("nfcapd", {}).get(
                        "latest_file_age_sec"
                    ),
                },
...
def api_performance_metrics():
    ...
    if metrics.get("subprocess_calls", 0) > 0:
        avg_subprocess_time = (
            metrics["subprocess_total_time"] / metrics["subprocess_calls"]
        )
        ...
        subprocess_stats = {
            ...
            "p95_ms": round(
                sorted(subprocess_times)[int(len(subprocess_times) * 0.95)] * 1000, 2
            )
            if len(subprocess_times) > 1
            else (round(subprocess_times[0] * 1000, 2) if subprocess_times else 0),
            "success_rate_percent": round(
                metrics.get("subprocess_success", 0)
                / metrics["subprocess_calls"]
                * 100,
                2,
            )
            if metrics.get("subprocess_calls", 0) > 0
...
def api_server_logs():
    ...
    return jsonify(
        {
            "logs": [
                "No logs available. Application logs will appear here as they are generated."
            ],
            "count": 1,
            "source": "none",
...
def _get_logs_from_files(lines):
    ...
    if all_lines:
        ...
        return jsonify({"logs": log_lines, "count": len(log_lines), "source": "files"})
    ...
    with _app_log_buffer_lock:
        if _app_log_buffer:
            ...
            log_lines.reverse()
            return jsonify(
                {"logs": log_lines, "count": len(log_lines), "source": "buffer"}
            )
        ...
        try:
                ...
                for table in tables:
                    try:
                        ...
                        for ts_col in [
                            ...
                            try:
                                cur = conn.execute(
                                    f"SELECT MIN({ts_col}) FROM {table} WHERE {ts_col} IS NOT NULL"
                                )
                                row = cur.fetchone()
            ...
            if len(history) >= 10:  # Need enough history for baseline
                ...
                if len(recent_samples) > 1:
                    recent_growth = (
                        recent_samples[-1]["file_size"] - recent_samples[0]["file_size"]
                    ) / max(len(recent_samples) - 1, 1)
                ...
                if len(baseline_samples) > 1:
                    baseline_growth = (
                        baseline_samples[-1]["file_size"]
                        - baseline_samples[0]["file_size"]
                    ) / max(len(baseline_samples) - 1, 1)
                ...
                elif baseline_growth <= 0 and recent_growth > (
                    stats["file_size"] * 0.01
                ):  # >1% of current size
                    write_pressure = "elevated"
...
def _clear_netflow_files():
    ...
    for name in os.listdir(NFCAPD_DIR):
        ...
        try:
            ...
            add_app_log(
                f"Maintenance: failed to remove NetFlow file '{name}': {repr(e)}",
                "ERROR",
            )
    ...
    add_app_log(
        f"Maintenance: clear_netflow_files removed={removed} bytes={removed_bytes} errors={len(errors)}",
        "INFO",
    )
    return {
...
def _restart_netflow_service():
    ...
    pid_file = os.environ.get("NFCAPD_PID_FILE") or os.path.join(
        nfcapd_dir, "nfcapd.pid"
    )
    try:
        os.makedirs(nfcapd_dir, exist_ok=True)
    except Exception as e:
        add_app_log(
            f"Maintenance: restart_netflow - Failed to access NetFlow dir '{nfcapd_dir}': {e}",
            "ERROR",
        )
    ...
    try:
        ...
        add_app_log(
            f"Maintenance: restart_netflow - Failed to start nfcapd with command {cmd}: {e}",
            "ERROR",
        )
...
def _restart_syslog_services():
    ...
    try:
        ...
        add_app_log(
            f"Maintenance: failed to restart syslog listener on port 514: {repr(e)}",
            "ERROR",
        )
        syslog_514 = {
            "status": "error",
            "message": "Failed to restart syslog listener on port 514.",
        }
    ...
    try:
        ...
        add_app_log(
            f"Maintenance: failed to restart firewall syslog listener on port 515: {repr(e)}",
            "ERROR",
        )
        syslog_515 = {
            "status": "error",
            "message": "Failed to restart firewall syslog listener on port 515.",
        }

    status = (
        "ok"
        if syslog_514.get("status") == "ok" and syslog_515.get("status") == "ok"
        else "partial"
    )
...
def api_tools_reputation():
    ...
    threat_feeds = (
        getattr(current_app, "threat_feeds", None) or threats_module.load_threatlist()
    )
...
def api_tools_shell():
    ...
    for pattern in blocked_patterns:
        if pattern in command_lower:
            return jsonify(
                {"error": f"Blocked: dangerous command pattern detected", "output": ""}
            ), 403
