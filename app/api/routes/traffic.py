...
def api_firewall_snmp_status():
    ...
    try:
        ...
        ip_addr_output = subprocess.check_output(ip_addr_cmd, stderr=subprocess.DEVNULL, timeout=3, text=True)
        ip_idx_output = subprocess.check_output(ip_idx_cmd, stderr=subprocess.DEVNULL, timeout=3, text=True)
        ...
        gw_output = subprocess.check_output(gw_cmd, stderr=subprocess.DEVNULL, timeout=3, text=True).strip()
        if gw_output and gw_output != '0.0.0.0':
            ...
            try:
                ping_output = subprocess.check_output(ping_cmd, stderr=subprocess.DEVNULL, timeout=2, text=True)
    ...
    for vpn_name, vpn_idx in vpn_interfaces.items():
        try:
            ...
            # Common OID list for errors/discards
            err_oids_list = [vpn_in_err_oid, vpn_out_err_oid, vpn_in_disc_oid, vpn_out_disc_oid]
            
            # Try 64-bit counters first, fallback to 32-bit if not available
            try:
                cmd = ["snmpget", "-v2c", "-c", snmp_community, "-Oqv", snmp_host, vpn_in_oid_hc, vpn_out_oid_hc, vpn_status_oid, vpn_speed_oid] + err_oids_list
                output = subprocess.check_output(cmd, stderr=subprocess.PIPE, timeout=3, text=True)
                ...
                cmd = ["snmpget", "-v2c", "-c", snmp_community, "-Oqv", snmp_host, vpn_in_oid_32, vpn_out_oid_32, vpn_status_oid, vpn_speed_oid] + err_oids_list
                output = subprocess.check_output(cmd, stderr=subprocess.PIPE, timeout=3, text=True)
                values = output.strip().split("\n")
