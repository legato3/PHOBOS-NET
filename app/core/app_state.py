...
_lock_attack_timeline = threading.Lock()
_lock_firewall_overview = threading.Lock()
_lock_network_overview = threading.Lock()
_lock_pkts = threading.Lock()
...
_stats_network_overview_cache = {"data": None, "ts": 0, "key": None}
_stats_firewall_overview_cache = {"data": None, "ts": 0, "key": None}
...
_stats_network_intelligence_cache = {"data": None, "ts": 0, "key": None}
...
_snmp_prev_sample = {
    "ts": 0
}  # Initialize with only timestamp; counter values will be None until first poll
...
_batch_executor = ThreadPoolExecutor(
    max_workers=20
)  # Dedicated pool for batch API requests
_nfdump_executor = ThreadPoolExecutor(
    max_workers=8
)  # Dedicated pool for nfdump subprocess calls
...
def update_thread_health(
    thread_name, success=True, execution_time_ms=None, error_msg=None
):
...
def get_thread_health_status():
    ...
    with _thread_health_lock:
        ...
        for name, th in _thread_health.items():
            ...
            avg_exec_ms = (
                round(sum(exec_times) / len(exec_times), 1) if exec_times else None
            )
            result[name] = {
                "status": status,
                "last_success": th["last_success"],
                "last_success_age_sec": round(now - th["last_success"], 1)
                if th["last_success"]
                else None,
    ...
    with _dependency_health_lock:
        ...
        for service, data in _dependency_health.items():
            ...
            if "response_times_ms" in result[service]:
                ...
                result[service]["avg_response_ms"] = (
            ...
            for key in [
                ...
                if key in result[service] and result[service][key]:
                    result[service][f"{key}_age_sec"] = round(
                        now - result[service][key], 1
                    )
    return result
...
def record_http_request_end(request_id, status_code, method, endpoint):
    ...
    with _http_metrics_lock:
        ...
        if request_id in _http_metrics["request_start_times"]:
            ...
            _http_metrics["concurrent_requests"] = max(
                0, _http_metrics["concurrent_requests"] - 1
            )
        if status_code >= 400:
    ...
    with _http_metrics_lock:
        return {
            ...
            "error_rate_percent": round(
                _http_metrics["total_errors"] / _http_metrics["total_requests"] * 100, 2
            )
            if _http_metrics["total_requests"] > 0
            else 0,
