...
def _trends_db_init():
    ...
    with _trends_db_lock:
        ...
        try:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_traffic_rollups_bucket ON traffic_rollups(bucket_end);"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_host_memory_first_seen ON host_memory(first_seen_ts);"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_host_metadata_updated_at ON host_metadata(updated_at);"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_db_size_history_db_path ON db_size_history(db_path, timestamp);"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_top_sources_bucket_end ON top_sources(bucket_end);"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_top_sources_ip ON top_sources(ip);"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_top_dests_bucket_end ON top_dests(bucket_end);"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_top_dests_ip ON top_dests(ip);"
            )
        ...
        try:
            ...
            try:
                ...
                if "rule_label" not in columns:
                    add_app_log(
                        "Migrating database: Adding rule_label column to fw_logs",
                        "INFO",
                    )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_syslog_program ON syslog_events(program)"
            )
...
def reset_firewall_database():
    ...
    with _firewall_db_lock:
        _firewall_db_initialized = False
        paths = [FIREWALL_DB_PATH, f"{FIREWALL_DB_PATH}-wal", f"{FIREWALL_DB_PATH}-shm"]
        for path in paths:
            try:
...
def _get_bucket_end(dt=None):
    ...
    current_bucket_end = dt.replace(
        minute=dt.minute - remainder, second=0, microsecond=0
    ) + timedelta(minutes=5)
...
def _ensure_rollup_for_bucket(bucket_end_dt):
    ...
    with _trends_db_lock:
        ...
        try:
            cur = conn.execute(
                "SELECT 1 FROM traffic_rollups WHERE bucket_end=?", (bucket_end_ts,)
            )
            row = cur.fetchone()
            if row:
                return
    ...
    with _trends_db_lock:
        ...
        try:
            conn.execute(
                "INSERT OR REPLACE INTO traffic_rollups(bucket_end, bytes, flows) VALUES (?,?,?)",
...
def get_host_memory(ip):
    ...
    with _trends_db_lock:
        ...
        try:
            ...
            row = cur.fetchone()
            if row:
                return {"first_seen_ts": row[0], "first_seen_iso": row[1]}
            return None
        finally:
            conn.close()
    ...
    with _trends_db_lock:
        ...
        try:
            ...
            for i in range(0, len(ip_list), batch_size):
                ...
                for row in cur.fetchall():
                    result[row[0]] = {"first_seen_ts": row[1], "first_seen_iso": row[2]}
        finally:
            conn.close()
...
def upsert_host_metadata(
    ip, tags=None, owner="", device_type="", criticality="", notes=""
):
...
def get_db_size_history(db_path, limit=100):
    ...
    with _trends_db_lock:
        ...
        try:
            ...
            return [
                {"file_size": row[0], "timestamp": row[1]} for row in cur.fetchall()
            ]
        finally:
            conn.close()
