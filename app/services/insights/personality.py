...
def generate_personality_profile(window="24h"):
    ...
    with _personality_lock:
        if (
            ...
            and _personality_cache["window"] == window
            and (now - _personality_cache["ts"] < _CACHE_TTL)
        ):
    ...
    try:
        top_ports = get_common_nfdump_data(
            "ports", window
        )  # List of dicts {key: "443", bytes: ...}
    except Exception:
        top_ports = []
    ...
    web_bytes = sum(
        p.get("bytes", 0) for p in top_ports if p.get("key") in ["443", "80", "8080"]
    )
    ...
    evidence["top_ports"] = (
        [
            {"port": int(p["key"]), "pct": round(p["bytes"] / total_bytes * 100, 1)}
            for p in top_ports[:3]
        ]
        if total_bytes > 0
        else []
    )
    evidence["block_rate_per_min"] = round(
        total_blocks / (24 * 60), 2
    )  # Rough calc for 24h
