...
def generate_overlays(window='1h'):
    ...
    for evt in events:
        ...
        if 'Firewall' in evt['source'] and ('Spike' in evt['title'] or 'High' in evt['title']):
            ...
            try:
                ...
                # Filter top 3
                evidence_dests = [{"ip": d['key'], "bytes": d['bytes']} for d in top_dests[:3]]
                ...
                overlays.append({
                    ...
                    "confidence": "medium",
                    "links": [
                         {"title": "View Traffic", "href": "/#section-worldmap;network"},
                         {"title": "Firewall Logs", "href": "/#section-summary;security"}
                    ],
        ...
        if 'SNMP' in evt['source'] and 'Utilization' in evt['title']:
             try:
                ...
                evidence_talkers = [{"ip": d['key'], "bytes": d['bytes']} for d in top_talkers[:3]]
                ...
                overlays.append({
                    ...
                    "confidence": "high",
                    "links": [
                         {"title": "View Top Sources", "href": "/#network"}
                    ],
        ...
        if 'New' in evt['title'] and 'Destination' in evt['title']:
             overlays.append({
                    ...
                    "confidence": "low",
                    "links": [
                         {"title": "Check Firewall", "href": "/#firewall"}
                    ],
                    "evidence": {"note": "Cross-reference newly visited IP with block logs"}
             })
