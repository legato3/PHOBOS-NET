    ...
    while not _shutdown_event.is_set() and not _syslog_thread_stop_event.is_set():
        try:
            ...
            if disk_info["percent_used"] > 90:
                print(
                    f"WARNING: NetFlow disk usage at {disk_info['percent_used']:.1f}% ({disk_info['used_gb']:.1f}GB / {disk_info['total_gb']:.1f}GB)"
                )
            elif disk_info["percent_used"] > 75:
                print(
                    f"INFO: NetFlow disk usage at {disk_info['percent_used']:.1f}% ({disk_info['used_gb']:.1f}GB / {disk_info['total_gb']:.1f}GB)"
                )
        except Exception as e:
