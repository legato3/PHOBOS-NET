...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not getattr(current_user, "is_admin", False):
            return jsonify(
                {"status": "error", "message": "Administrator privileges required"}
            ), 403
        return f(*args, **kwargs)
        ...
        def wrapper(*args, **kwargs):
            ...
            with lock:
                ...
                if (
                    ...
                    and cache_dict.get("key") == cache_key
                    and (now - cache_dict.get("ts", 0)) < ttl_seconds
                ):
        ...
        def wrapper(*args, **kwargs):
            ...
            with _throttle_lock:
                _request_times[endpoint] = [
                    t for t in _request_times[endpoint] if now - t < time_window
                ]
