...
from app.config import (
    ...
    THREATLIST_PATH,
    THREAT_FEEDS_PATH,
    THREAT_FEED_URL_PATH,
    MITRE_MAPPINGS,
    SECURITY_WEBHOOK_PATH,
    WEBHOOK_PATH,
    PORTS,
    SUSPICIOUS_PORTS,
    BRUTE_FORCE_PORTS,
    ...
    PORT_SCAN_WINDOW,
    EXFIL_THRESHOLD_MB,
    EXFIL_RATIO_THRESHOLD,
    DNS_QUERY_THRESHOLD,
    BUSINESS_HOURS_START,
    BUSINESS_HOURS_END,
    OFF_HOURS_THRESHOLD_MB,
    VIRUSTOTAL_API_KEY,
    ABUSEIPDB_API_KEY,
)
    ...
    try:
        ...
        with requests.Session() as session:
            ...
            adapter = requests.adapters.HTTPAdapter(
                pool_connections=MAX_FEED_WORKERS, pool_maxsize=MAX_FEED_WORKERS
            )
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for url, category, name in feed_entries:
                    ...
                    future = executor.submit(
                        _process_feed, url, category, name, last_ok, session
                    )
        ...
        _threat_status["feeds_ok"] = sum(
            1 for f in new_feed_status.values() if f["status"] == "ok"
        )
        ...
        add_app_log(
            f"Threat feed update completed in {duration:.2f}s ({_threat_status['size']} IPs)",
            "INFO",
        )
...
def get_threat_timeline(ip):
    ...
    with _threat_timeline_lock:
        return _threat_timeline.get(
            ip, {"first_seen": 0, "last_seen": 0, "hit_count": 0}
        )
...
def load_security_webhook_config():
    ...
    try:
        ...
        if (
            mtime != _webhook_config_cache["mtime"]
            or _webhook_config_cache["data"] is None
        ):
        ...
        with _webhook_start_lock:
            if _webhook_thread is None or not _webhook_thread.is_alive():
                _webhook_thread = threading.Thread(
                    target=_process_webhooks, daemon=True, name="SecurityWebhookWorker"
                )
...
def detect_anomalies(
                    ...
                    alerts.append(
                        {
                            ...
                            "msg": f"Suspicious port {port} ({service}): {fmt_bytes(item['bytes'])}",
                            "severity": "high",
                            "feed": "local",
                        }
                    )
    ...
    datasets = [(sources_data, "from", True), (destinations_data or [], "to", False)]
...
def detect_data_exfiltration(sources_data, destinations_data):
    ...
    for ip, traffic in internal_traffic.items():
        ...
        if out_mb >= EXFIL_THRESHOLD_MB:
            ...
            ratio >= EXFIL_RATIO_THRESHOLD and out_mb >= 50
        ):  # At least 50MB with high ratio
            alerts.append(
                {
                    "type": "data_exfil",
    ...
    with _alert_history_lock:
        ...
        if matching_alert:
            ...
            if "created_at" not in matching_alert:
                matching_alert["created_at"] = matching_alert.get(
                    "first_seen", matching_alert.get("ts", now_ts)
                )
...
def lookup_threat_intelligence(ip):
    ...
    result = {"enabled": False, "virustotal": None, "abuseipdb": None}
...
def detect_ip_anomalies(ip, tf, direction, src_ports, dst_ports, protocols):
    ...
    try:
        ...
        unique_ports = len(
            set(
                [p.get("key", "") for p in (src_ports or [])]
                + [p.get("key", "") for p in (dst_ports or [])]
            )
        )
        if unique_ports > 20:
            anomalies.append(
...
def generate_ip_anomaly_alerts(ip, anomalies, geo):
    ...
    with _alert_history_lock:
        ...
        recent_alerts = [
            a for a in recent_history if a.get("ip") == ip and a.get("ts", 0) > hour_ago
        ]
        if recent_alerts:
