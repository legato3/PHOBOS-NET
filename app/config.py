...
APP_NAME = "PHOBOS-NET"
APP_VERSION = "v2.1.0"
APP_VERSION_DISPLAY = "v2.1.0"  # Human-readable version for UI
...
OBS_NFDUMP_WARN_MS = float(
    os.getenv("OBS_NFDUMP_WARN_MS", "5000")
)  # Warn if nfdump > 5s
OBS_CACHE_MISS_RATE_WARN = float(
    os.getenv("OBS_CACHE_MISS_RATE_WARN", "0.5")
)  # Warn if miss rate > 50%
OBS_ROUTE_SLOW_MS = float(
    os.getenv("OBS_ROUTE_SLOW_MS", "1000")
)  # Flag route as slow if > 1s
OBS_ROUTE_SLOW_WARN_MS = float(
    os.getenv("OBS_ROUTE_SLOW_WARN_MS", "2000")
)  # Warn if route > 2s
OBS_SERVICE_SLOW_MS = float(
    os.getenv("OBS_SERVICE_SLOW_MS", "500")
)  # Warn if service function > 500ms
...
THREAT_FEEDS_PATH = os.environ.get(
    "THREAT_FEEDS_PATH", "/opt/phobos/config/threat-feeds.txt"
)
THREAT_FEED_URL_PATH = os.environ.get(
    "THREAT_FEED_URL_PATH", "/app/data/threat-feed.url"
)
...
SECURITY_WEBHOOK_PATH = os.environ.get(
    "SECURITY_WEBHOOK_PATH", "/app/data/security-webhook.json"
)
...
FIREWALL_IP = os.getenv(
    "FIREWALL_IP", "0.0.0.0"
)  # 0.0.0.0 = accept from any source (restrict via env var in production)
...
try:
    LONG_LOW_DURATION_THRESHOLD = float(
        os.getenv("LONG_LOW_DURATION_THRESHOLD", "300")
    )  # 5 minutes default
...
try:
    LONG_LOW_BYTES_THRESHOLD = int(
        os.getenv("LONG_LOW_BYTES_THRESHOLD", "100000")
    )  # 100 KB default
except (ValueError, TypeError):
...
BASELINE_WINDOW_SIZE = int(
    os.getenv("BASELINE_WINDOW_SIZE", "100")
)  # Number of samples in rolling window
BASELINE_UPDATE_INTERVAL = int(
    os.getenv("BASELINE_UPDATE_INTERVAL", "300")
)  # Update every 5 minutes (300s)
BASELINE_DEVIATION_MULTIPLIER = float(
    os.getenv("BASELINE_DEVIATION_MULTIPLIER", "2.0")
)  # 2 standard deviations = abnormal
