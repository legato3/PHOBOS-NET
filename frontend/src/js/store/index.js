...
export const Store = () => ({
    ...
    get friendlyLabels() {
        return (typeof DashboardWidgets !== 'undefined' && DashboardWidgets.friendlyLabels) || {
            ...
            flags: 'TCP Flags (Sampled)',
            asns: 'Top ASNs',
            countries: 'Traffic by Country',
            durations: 'Long Flows',
            packetSizes: 'Packet Sizes (Sampled)',
            sources: 'Top Sources',
        ...
        try {
            ...
            if (res.ok) {
                ...
                this.appMetadata = {
                    ...
                    version_display: data.version_display || 'v2.1.0'
                };
            }
        } catch (e) {
    ...
    openStatDetail(kind) {
        ...
        if (kind === 'networkState') {
            ...
            base.components = [
                { label: 'NetFlow', status: health.netflow ? 'Active' : 'Missing', color: health.netflow ? 'var(--signal-ok)' : 'var(--signal-crit)' },
                { label: 'Syslog (514)', status: health.syslog ? 'Active' : 'Missing', color: health.syslog ? 'var(--signal-ok)' : 'var(--signal-warn)' },
                { label: 'Firewall Logs', status: health.firewall_syslog ? 'Active' : 'Missing', color: health.firewall_syslog ? 'var(--signal-ok)' : 'var(--signal-warn)' },
